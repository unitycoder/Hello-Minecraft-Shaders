...
void main() {
    ...
    color.rgb = mix(color.rgb, bloom.rgb, vec3(0.05));
    ...
    for(int i=-1; i<=1; i++) {
        for(int j=-1; j<=1; j++) {
            vec2 offset = vec2(i/viewWidth,j/viewHeight) * 2;
            cloud += texture2D(colortex3, texcoord.st * 0.5 + offset);
            weight += 1;
        }
    }
    cloud /= weight;
    color.rgb = mix(color.rgb, cloud.rgb, clamp(cloud.a, 0, 1));
    //color.rgb = texture2D(colortex3, texcoord.st * 0.5).rgb;
